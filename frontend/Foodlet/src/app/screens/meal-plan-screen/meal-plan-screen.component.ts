...
export class MealPlanScreenComponent {
  ...
  //TODO: Get meal plan from database. (v)
  ...
  findMealPlanForWeek(weekNumber: number): MealPlan[] {
    let mealPlans = this.mealPlan.filter(
      (mealPlan) => this.getWeekNumber(mealPlan.date) === weekNumber
    ); //find meal plans for the current week

    if (mealPlans.length == 7) {
      //if there are 7 meal plans for the week, return them
      this.weekHasMealPlan = true;
      return mealPlans;
    } else if (mealPlans.length > 7) {
      //if there are more than 7 meal plans for the week, return the first 7
      this.weekHasMealPlan = true;
      return mealPlans.slice(0, 7);
    } else if (mealPlans.length === 0) {
      //if there are no meal plans for the week, return an empty array
      this.weekHasMealPlan = false;
      return [];
    } else {
      //else find the missing days
      let missingDays: Date[] = [];
      this.getDaysOfWeek(new Date().getFullYear(), weekNumber).forEach(
        (day) => {
          if (
            !mealPlans.find(
              (mealPlan) => mealPlan.date.getDay() === day.getDay()
            )
          ) {
  ...
  getWeekNumber(date: Date) {
    ...
    if (tdt.getDay() !== 4) {
      //not Thursday? get next Thursday
  ...
  getDaysOfWeek(year: number, weekNumber: number): Date[] {
    ...
    // 1st day is the day of the month - the day of the week
    else ISOweekStart.setDate(simple.getDate() + 8 - simple.getDay()); // 1st day is the day of the month + 8 - the day of the week
  ...
  generateMealPlan() {
    let days: Date[] = this.getDaysOfWeek(
    ...
    this.dialog
      .open(MaxNutrientsDialogComponent, {
